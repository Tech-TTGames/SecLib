...
def send_recovery(user):
    ...
    send_email('[SecLib] Account Recovery',sender=current_app.config['ADMINS'][0],recipients=[user.email]
...
def send_activate(user):
    token = user.get_token()
    send_email('[SecLib] Account Activation',sender=current_app.config['ADMINS'][0],recipients=[user.email]
